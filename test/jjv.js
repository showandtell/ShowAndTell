...
(function () {
  ...
  var checkValidity = function (env, root_schema, schema, object, name, options) {
    ...
    var i, len, count, orig_object = {}, refs, prop, hasProp, hasPattern;
    ...
    for (v in schema) {
      if (schema.hasOwnProperty(v) && !handled.hasOwnProperty(v)) {
        if (v === 'format') {
          if (env.fieldFormat.hasOwnProperty(schema[v]) && !env.fieldFormat[schema[v]](prop, schema, name, options)) {
            objerrs[v] = true;
            malformed = true;
          }
        } else {
          if (env.fieldValidate.hasOwnProperty(v) && !env.fieldValidate[v](prop, schema[v], schema, name, options)) {
            objerrs[v] = true;
            malformed = true;
          }
        }
  ...
  Environment.prototype = {
    validate: function (name, object, options) {
      var refs, errors = null, root_object = {'__root__': object};
      ...
      errors = checkValidity(this, refs[0], refs[1], root_object, '__root__', options);
